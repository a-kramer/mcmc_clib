 ...
 *
 *  Most recent changes break this mechanism, this file needs to be fixed
 */
#include <dlfcn.h>
#ifndef _GNU_SOURCE
#include <libgen.h>
...
#define	ODE_SOLVER_MAX_STEPS		5000
...
ode_solver* ode_solver_alloc(ode_model* model){ 
  ...
  solver->solver_mem = CVodeCreate(CV_BDF,CV_NEWTON);
  if( solver->solver_mem == NULL){
    fprintf(stderr,"CVodeCreate failed to allocate memory in ode_solver for solver_mem.\n");
    free(solver);
    return NULL;
  }  
...
void ode_solver_setErrTol(ode_solver* solver, const double rel_tol,  double* abs_tol, const int abs_tol_len){
  ...
  /* set tollerances to the solver_mem internal structure */
  if ( abs_tol_len == 1 )
    CVodeSStolerances(solver->solver_mem, rel_tol, abs_tol[0]);
  else{
    N_Vector abs_tol_vec =  N_VNewEmpty_Serial(abs_tol_len);		/* alloc */
    NV_DATA_S(abs_tol_vec) = abs_tol;
    CVodeSVtolerances(solver->solver_mem, rel_tol, abs_tol_vec);
...
void ode_solver_init(ode_solver* solver, const double t0,  double* y0, int lenY, double* p, int lenP ){
  ...
  /* initialise */
  flag = CVodeInit(solver->solver_mem, solver->odeModel->vf_eval, t0, solver->y);
  flag |= CVodeSetUserData(solver->solver_mem, solver->params);
  flag |= CVDense(solver->solver_mem, solver->odeModel->N);
  //flag |= CVDlsSetJacFn(solver->solver_mem, solver->odeModel->vf_jac);
  flag |= CVDlsSetDenseJacFn(solver->solver_mem, solver->odeModel->vf_jac);
  flag |= CVodeSStolerances(solver->solver_mem, ODE_SOLVER_REL_ERR, ODE_SOLVER_ABS_ERR);
  flag |= CVodeSetMaxNumSteps(solver->solver_mem, ODE_SOLVER_MX_STEPS);
  if (flag!=CV_SUCCESS) {
...
void ode_solver_reinit(ode_solver* solver, const double t0,  double* y0, int lenY,  const double* p, int lenP ){
  ...
  /* re-initialise */
  flag = CVodeReInit(solver->solver_mem, t0, solver->y);
...
void ode_solver_init_sens(ode_solver* solver,  double* yS0, int lenP, int lenY){
  ...
  flag = CVodeSensInit1(solver->solver_mem, lenP, CV_STAGGERED1, solver->odeModel->vf_sens, solver->yS);
  flag &= CVodeSetSensErrCon(solver->solver_mem, 1); /* 1 is "true" */	
  flag &= CVodeSensEEtolerances(solver->solver_mem);
  ...
  flag &= CVodeSetSensParams(solver->solver_mem, solver->params, scale_p, NULL);
  if (flag!=CV_SUCCESS) {
    fprintf(stderr,"[CV] ode_solver_init failed, flag=%i\n",flag);
  }
}
...
void ode_solver_disable_sens(ode_solver* solver){
  CVodeSensToggleOff(solver->solver_mem);
}
...
void ode_solver_reinit_sens(ode_solver* solver,  double* yS0, int lenP, int lenY){
  ...
  flag = CVodeSensReInit(solver->solver_mem, CV_STAGGERED1, solver->yS);
  flag &= CVodeSetSensParams(solver->solver_mem, solver->params, scale_p, NULL);
  if (flag!=CV_SUCCESS){
...
int ode_solver_solve(ode_solver* solver, const double t, double* y, double* tout){
  ...
  int flag = CVode(solver->solver_mem,t, solver->y, &lTout, CV_NORMAL);
...
void ode_solver_get_sens(ode_solver* solver, double t, double* yS){
  ...
  if (solver->odeModel->vf_sens){
    ...
    CVodeGetSens(solver->solver_mem, &t, solver->yS);
  }
}
...
void ode_solver_print_stats(const ode_solver* solver, FILE* outF){
  ...
  void* solver_mem = solver->solver_mem;
  
  flag = CVodeGetNumSteps(solver_mem, &nst);
  flag |= CVodeGetNumRhsEvals(solver_mem, &nfe);
  flag |= CVodeGetNumLinSolvSetups(solver_mem, &nsetups);
  flag |= CVodeGetNumErrTestFails(solver_mem, &netf);
  flag |= CVodeGetNumNonlinSolvIters(solver_mem, &nni);
  flag |= CVodeGetNumNonlinSolvConvFails(solver_mem, &ncfn);
  
  if (solver->yS) {
    flag |= CVodeGetSensNumRhsEvals(solver_mem, &nfSe);
    flag |= CVodeGetNumRhsEvalsSens(solver_mem, &nfeS);
    flag |= CVodeGetSensNumLinSolvSetups(solver_mem, &nsetupsS);
    flag |= CVodeGetSensNumErrTestFails(solver_mem, &netfS);
    flag |= CVodeGetSensNumNonlinSolvIters(solver_mem, &nniS);
    flag |= CVodeGetSensNumNonlinSolvConvFails(solver_mem, &ncfnS);
  }
