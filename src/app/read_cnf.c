...
int normalise_by_timepoint_with_sd(ode_model_parameters *omp, problem_size *ps, gsl_matrix_int *normalisation){
  ...
  for (c=0;c<C;c++){
    for (j=0;j<T;j++) {
      ...
      gsl_vector_add(sd_d,tmp); // so: sd_d*(1/s) + sd_s*(d/s²)
    }
  }
  gsl_free(tmp);
  return GSL_SUCCESS;  
...
int normalise_by_state_var_with_sd(gsl_matrix_sd *A, gsl_matrix_int *normalisation){
   ...
   */
  int i,j,l,k;
  int n=A->M->size1,m=A->M->size2;
  gsl_vector *tmp; // intermediate results
  gsl_vector *r,*d,*sd; // line by line view of the data
  gsl_vector_view R,D,SD;
  double s;

  gsl_vector_alloc(tmp,m);
  for (i=0;i<n;i++) {
    D=gsl_matrix_row(A->M, i);
    d=&(D.vector);
    SD=gsl_matrix_row(A->sd, i);
    sd=&(SD.vector);
    l=gsl_matrix_int_get(normalisation,1,i);
    k=gsl_matrix_int_get(normalisation,2,i);
    R=gsl_matrix_row(A->M, k);
    r=&(R.vector);
    r_sd=&(SR.vector);
    s=gsl_vector_get(r,l); // data will be scaled by 1/s
    s_sd=gsl_vector_get(r_sd,l);
    gsl_vector_scale(d,1.0/s);
    // calculate standard deviation of result:
    gsl_vector_set_zero(tmp);
    gsl_vector_add(tmp,d);
    // add both error terms
    gsl_vector_scale(tmp,s_sd/s);  // so: s_sd × d/(s²)  
    gsl_vector_mul(sd,d);
    gsl_vector_add(sd,tmp);
  }
  gsl_free(tmp);
  return GSL_SUCCESS;  
}
