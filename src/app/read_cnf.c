...
int gsl_object_free(gsl_object *G){
  if (G->is_matrix){
    if (G->is_double) {
      ...
      printf("gsl_matrix is neither int nor double.\n");
      exit(-1);
    }
  }  else if (G->is_vector){
    if (G->is_double) {
      ...
      printf("gsl_vector is neither int nor double.\n");
      exit(-1);
    }
  } else {
    printf("gsl_object is neither matrix nor vector.\n");
    exit(-1);
  }
  free(G->gsl);
  free(G);
...
int read_problem_definition(FILE *cnf, ode_model_parameters *omp, const field_expression *fe, const regex_t *comment, main_options *cnf_options){
  ...
  var_ndim_array *nda;
  gsl_object *G;
  gsl_matrix_sd Data={NULL,NULL};
  gsl_matrix_sd ReferenceData={NULL,NULL};
  ...
  do {
	...
	if (C==0){
	  fprintf(stderr,"number of experiments (C) is not specified\n"); exit(-1);
	}
	...
	case i_norm_f: // norm_f and norm_t can be combined, so they add up.
	  ...
	  nda=read_block_nd(cnf,INTEGER_BLOCK,current->closing_bracket,comment);
	  gsl_object_init(G);
	  nda_to_gsl(nda,G);
	...
	case i_norm_t: // mixing reference data and these two types will then lead to an error.
	  ...
	  status=gsl_matrix_int_memcpy(omp->norm_t, G->gsl->matrix_int);
  ...
  for (i=0;i<C;i++){
    if (omp->E[i]->t!=NULL){
      ...
      fprintf(stderr,"measurementy times missing.\n");
      exit(-1);
    }    
    r+=T;
  }
  ...
  // print some read in blocks:
  /*
  printf("norm_f:[\n");
  for (i=0;i<omp->norm_f->size1;i++){
    for (j=0;j<omp->norm_f->size2;j++) printf(" %i ",gsl_matrix_int_get(omp->norm_f,i,j));
    printf("\n");
  }
  printf("]\n");
  fflush(stdout);
  */
  ...
  fflush(stdout);
  gsl_matrix_free(Data.M); printf(".");   fflush(stdout);
  gsl_matrix_free(Data.sd); printf(".");  fflush(stdout);
  if (ReferenceData.M!=NULL && ReferenceData.sd!=NULL){
    gsl_matrix_free(ReferenceData.M); printf(".");  fflush(stdout);
    gsl_matrix_free(ReferenceData.sd); printf(".");  fflush(stdout);
  }
  printf("done.\n");
  fflush(stdout);
  gsl_object_free(G);
