...
void VectorField::PrintESENS(map<string,string> options){
	...
	matrix Jf = getJacobian(funcvecfield_list, expreqn_list, varname_list);
	
	/* create derivatives of the original system */
	matrix Jp = getJacobian(funcvecfield_list, expreqn_list, parname_list);
	...
	/*add function sensitivities*/
	/* calculate matrix with the rhs of the sensitivity equations */
	/* dot(CS) = C( Jy * S + Jp ) */
	matrix C = getJacobian(funcvecfield_list, expreqn_list, funcname_list);
	matrix dfV = C.mul(Jy.mul(S).add(Jp)); // how to apply function to
					// sensitivities: since
					// functions are linear the
					// Jacobian will do the trick
