...
void VectorField::PrintPyGSL(map<string,string> options)
    ...
    string numstr = "numpy";
    if (options["num"] == "numeric")
        numstr = "Numeric";
    fout << "import " << numstr << endl;
    fout << endl;
    //
    ...
    fout << endl;
    if (options["num"] == "numeric")
        fout << "    f_ = Numeric.zeros((" << nv << ",),Numeric.Float)" << endl;
    else
    ...
    if (options["num"] == "numeric")
        fout << "    jac_ = Numeric.zeros((" << nv << "," << nv << "),Numeric.Float)" << endl;
    else
    ...
    fout << endl;
    if (options["num"] == "numeric")
       fout << "    dfdt_ = Numeric.zeros((2,),Numeric.Float)" << endl;
    else
       fout << "    dfdt_ = numpy.zeros((2,))" << endl;
