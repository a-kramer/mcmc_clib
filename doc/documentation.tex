...
\begin{align}
  \dot x &= f(t,x;\theta,u)\,,\\
  y(t_j;\theta,u)&=C x(t_j;\theta,u) + \epsilon_i(t_j)\,,
\end{align}
...
\begin{equation}
  ...
  \end{pmatrix}\,.
\end{equation}
Assuming that $C$ is columnwise equally scaled in this way, with unknown
coefficients $c_i$, we can eliminate these unknwon elements by taking
rowwise ratios:
\begin{align}
