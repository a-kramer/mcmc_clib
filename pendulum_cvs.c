 ...
 *  Generated on 23-Aug-2017 at 09:26
 */
...
int pendulum_vf(realtype t, N_Vector y_, N_Vector f_, void *params)
    ...
    NV_Ith_S(f_,0) = v;
    NV_Ith_S(f_,1) = -v/m*b/(L*L)-g*sin(theta)/L;
    return 0;
    }
...
int pendulum_jac(int N_, realtype t,
    ...
    DENSE_ELEM(jac_, 1, 0) = -g*cos(theta)/L;
    DENSE_ELEM(jac_, 1, 1) = -1.0/m*b/(L*L);
    return 0;
    }
/*
...
int pendulum_jacp(int N_, realtype t,
    ...
    DENSE_ELEM(jacp_, 1, 1) = -v/m/(L*L);
    DENSE_ELEM(jacp_, 1, 2) =  g*sin(theta)/(L*L)+2.0*v/m*b/(L*L*L);
    DENSE_ELEM(jacp_, 1, 3) = v/(m*m)*b/(L*L);
    return 0;
    }
...
int pendulum_sens(int Ns_, realtype t, N_Vector y_, N_Vector ydot_,
    ...
    NV_Ith_S(ySdot_,1) = -1.0/m*S1*b/(L*L)-S0*g*cos(theta)/L;
	...
	switch (iS_) {
        case 0:
	        ...
	        break;
        case 1:
	            NV_Ith_S(ySdot_,1) += -v/m/(L*L);
	        break;
        case 2:
	            NV_Ith_S(ySdot_,1) +=  g*sin(theta)/(L*L)+2.0*v/m*b/(L*L*L);
	        break;
        case 3:
	            NV_Ith_S(ySdot_,1) += v/(m*m)*b/(L*L);
	        break;
	 }
    return 0;
    }
...
int pendulum_func(realtype t, N_Vector y_, realtype *func_, void *params)
    ...
    /* energy:  */
    func_[0] =  (v*v)*m*(L*L)/2.0-m*g*cos(theta)*L;
    return 0;
    }
...
int pendulum_func_sens(realtype t, N_Vector y_, N_Vector * yS_, double* ret_, void *params)
    ...
    /* g:  */
        ret_[0] = -m*cos(theta)*L+v*m*(L*L)*yS_01+m*g*yS_00*sin(theta)*L;
    /* b:  */
        ret_[1] =  v*m*yS_11*(L*L)+m*g*yS_10*sin(theta)*L;
    /* L:  */
        ret_[2] =  m*g*sin(theta)*L*yS_20-m*g*cos(theta)+v*m*yS_21*(L*L)+(v*v)*m*L;
    /* m:  */
        ret_[3] =  v*m*yS_31*(L*L)+m*g*yS_30*sin(theta)*L+(v*v)*(L*L)/2.0-g*cos(theta)*L;
    return 0;
    }
